...
const joinSchema = z
  .object({
    ...
    birth: z
      ...
      .trim()
      .min(1, "생년월일을 입력해주세요.")
      .regex(/^\d{4}-\d{2}-\d{2}$/, "올바른 날짜 형식으로 입력해주세요.")
      .refine((date) => {
...
const JoinFormPage = () => {
  ...
  const hashDeleteHandler = (id: number) => {
    ...
    setValue(
      "interests",
      interestList.filter((item) => item !== id)
    );
    setValue(`checkbox-${id}`, false);
  };

  // 체크박스 클릭 시 값 업데이트
  const handleCheckboxChange = (e: FormEvent<HTMLInputElement>, id: number) => {
    const target = e.target as HTMLInputElement;
    const name = target.value;
    const type = target.checked;

    if (type) {
      setInterestList((prev) => [...prev, id]);
  ...
  useEffect(() => {
    setValue("interests", interestList);
  }, [interestList, setValue, favorList]);
  ...
  return (
    <JoinFormWrap>
      <JoinForm onSubmit={onSubmit}>
                        ...
                        category.map((data) => {
                          return (
                            <InputCheckboxB
                              key={data.id}
                              label={data.name}
                              id={`checkbox-${data.id}`} // id를 구분하여 설정
                              // checked={watch(`checkbox-${data.id}`)} //
                              onChange={(e) => {
        ...
        {errors.interests && (
          <p className="err_msg">{errors.interests?.message?.toString()}</p>
        )}
        <AgreeBox>
          <InputCheckboxB
